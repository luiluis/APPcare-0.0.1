...
import { Activity, Heart, TrendingUp, TrendingDown, Minus, Plus, X, AlertTriangle, Scale, Droplets } from 'lucide-react';
...
export const ClinicalTab: React.FC<ClinicalTabProps> = ({ resident, onUpdateResident }) => {
    ...
    onUpdateResident({
      ...
      medicalRecord: {
        ...
        lastVitals: {
          ...
          glucose: vitalsForm.glucose ? vitalsForm.glucose : medicalRecord.lastVitals.glucose, 
          glucoseTrend: 'stable',
  ...
  const cardStyles = {
      ...
      alert: "bg-red-50 border-red-200 animate-pulse"
  };
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm">
           <div className="flex justify-between items-center mb-6">
              <h3 className="font-semibold text-gray-800 flex items-center gap-2">
                <Heart className="w-5 h-5 text-rose-500" /> Monitoramento de Sinais Vitais
              </h3>
              <div className="flex items-center gap-3">
                 <span className="text-xs text-gray-400 font-medium">Última: {medicalRecord.lastVitals.date}</span>
                 <button onClick={handleOpenVitals} className="flex items-center gap-1 px-4 py-2 bg-rose-50 text-rose-700 hover:bg-rose-100 rounded-lg text-xs font-bold transition-colors">
                   <Plus className="w-3 h-3" /> Nova Aferição
                 </button>
              </div>
           </div>
           ...
           <div className="grid grid-cols-3 gap-4">
             ...
             <div className={`p-4 rounded-xl text-center border ${cardStyles[bpStatus]}`}>
                <p className="text-xs text-gray-500 font-bold uppercase mb-2">Pressão Arterial</p>
                <div className="flex items-center justify-center gap-2">
                   <p className={`text-3xl font-bold ${textStyles[bpStatus]}`}>{medicalRecord.lastVitals.pressure}</p>
             ...
             <div className={`p-4 rounded-xl text-center border ${cardStyles[glStatus]}`}>
                <p className="text-xs text-gray-500 font-bold uppercase mb-2">Glicemia (HGT)</p>
                <div className="flex items-center justify-center gap-2">
                   <p className={`text-3xl font-bold ${textStyles[glStatus]}`}>{medicalRecord.lastVitals.glucose}</p>
                </div>
             ...
             <div className="bg-gray-50 border border-gray-100 p-4 rounded-xl text-center">
                <p className="text-xs text-gray-500 font-bold uppercase mb-2">Frequência Cardíaca</p>
                <div className="flex items-center justify-center gap-2">
                   <p className="text-3xl font-bold text-gray-900">{medicalRecord.lastVitals.heartRate}</p>
                </div>
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm">
           <h3 className="font-semibold text-gray-800 mb-6 flex items-center gap-2">
              <Scale className="w-5 h-5 text-emerald-600" /> Acompanhamento Nutricional & Antropometria
           </h3>
           <div className="grid grid-cols-3 gap-6 divide-x divide-gray-100">
               <div className="text-center px-2">
                   <p className="text-xs text-gray-400 uppercase font-bold mb-2">Peso Atual</p>
                   <p className="text-2xl font-bold text-gray-900">{medicalRecord.weight ? `${medicalRecord.weight} kg` : '-'}</p>
               </div>
               <div className="text-center px-2">
                   <p className="text-xs text-gray-400 uppercase font-bold mb-2">Altura</p>
                   <p className="text-2xl font-bold text-gray-900">{medicalRecord.height ? `${medicalRecord.height} m` : '-'}</p>
               </div>
               <div className="text-center px-2">
                   <p className="text-xs text-gray-400 uppercase font-bold mb-2">IMC Calculado</p>
                   ...
                   {medicalRecord.bmi && medicalRecord.bmi < 18.5 && <p className="text-[10px] text-amber-600 font-bold mt-1">Baixo Peso</p>}
                   {medicalRecord.bmi && medicalRecord.bmi > 25 && <p className="text-[10px] text-amber-600 font-bold mt-1">Sobrepeso</p>}
               </div>
           </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
         ...
         <div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm flex items-center justify-between">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-xl font-bold text-gray-900">{medicalRecord.bloodType || 'N/A'}</p>
                </div>
            </div>
         </div>
         ...
         <div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm">
            <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2">
                <Activity className="w-5 h-5 text-blue-500" /> Diagnósticos
            </h3>
            <div className="flex flex-wrap gap-2">
                {medicalRecord.comorbidities.length > 0 ? medicalRecord.comorbidities.map(item => (
                <span key={item} className="px-3 py-1.5 bg-blue-50 text-blue-800 font-medium rounded-lg text-xs border border-blue-100">
                    {item}
                </span>
                )) : <span className="text-gray-400 text-sm italic">Nenhum diagnóstico registrado.</span>}
            </div>
         </div>

         {/* Alergias */}
         <div className="bg-red-50 border border-red-100 rounded-xl p-6 shadow-sm">
             <h4 className="text-sm font-bold text-red-800 uppercase mb-4 flex items-center gap-2"><AlertTriangle className="w-4 h-4"/> Alergias</h4>
             <div className="flex flex-wrap gap-2">
                {medicalRecord.allergies.length > 0 ? medicalRecord.allergies.map(item => (
                  <span key={item} className="px-2 py-1 bg-white text-red-700 font-bold rounded-md text-xs border border-red-200 shadow-sm">
                    {item}
                  </span>
                )) : <span className="text-red-400 text-xs italic">Nenhuma alergia conhecida.</span>}
             </div>
         </div>
      </div>
